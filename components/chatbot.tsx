...
function findAnswer(input: string): { answer: string; followUps: string[] } {
  ...
  for (const qa of QA_DATA) {
    if (qa.keywords.some((kw) => lower.includes(kw))) {
      return {
        answer: qa.answer,
        followUps: qa.followUps ?? FALLBACK_FOLLOWUPS,
      };
    }
  }
          ...
          >
            ...
            >
              {messages.map((msg, i) => (
                ...
                >
                  <div
                    className={`max-w-[82%] px-4 py-2.5 text-[13.5px] leading-relaxed rounded-2xl whitespace-pre-line ${
                      msg.role === "user"
                ...
                {isTyping && (
                  ...
                  >
                    <div className="flex items-center gap-1.5 px-4 py-3 bg-black/4 rounded-2xl rounded-bl-md">
                      <span
                        className="w-2 h-2 rounded-full bg-brand/50 typing-dot"
