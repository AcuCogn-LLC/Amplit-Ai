...
const CARDS = [
    ...
    },
    {
        ...
        image: '/trust-image/conversations.png',
    }
];

const BOTTOM_CARD = {
    subheading: 'Advanced Analytics',
    heading: 'Revenue Intelligence and Realtime Analytics',
    description:
        'Know exactly how calls convert into bookings and identify where opportunities are being lost. Gain real-time insights into call volume, peak hours, and overall operational efficiency. ',
    image: '/images/Dashboard-bg.png',
    imageAlt: 'Advanced Analytics platform',
};
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 relative z-10">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8">
                    ...
                    <motion.div
                        ...
                        className="md:col-span-3 bg-white border border-brand/60 rounded-3xl p-6 lg:p-10 pb-0 lg:pb-0 flex flex-col relative overflow-hidden group"
                    >
                        
                            <div className="absolute bottom-0 left-0 right-0 h-48 bg-linear-to-t from-brand/10 to-transparent pointer-events-none" />

                           {/* Two-column text below */}
                        <div className="flex flex-col md:flex-row gap-6 md:gap-16 mb-8 items-start">
                            <div className="w-full md:w-1/2">
                                {/* <p className="text-[#4e5157] font-medium text-[13px] md:text-sm mb-3 tracking-wide">{BOTTOM_CARD.subheading}</p> */}
                                <h3 className="text-[24px] md:text-[32px] font-bold text-gray-900 leading-[1.3]">{BOTTOM_CARD.heading}</h3>
                            </div>
                            <div className="w-full md:w-1/2 flex flex-col justify-center">
                                <p className="text-gray-600 text-[15px] md:text-[17px] leading-relaxed">
                                    {BOTTOM_CARD.description}
                                </p>
                            </div>
                        </div>

                        {/* Full-width Image - bottom touching */}
                        <div className="relative w-full mt-auto z-10 translate-y-4 group-hover:translate-y-2 transition-transform duration-500 ease-out">
                            <div className="w-full bg-white rounded-t-2xl shadow-[0_0_30px_rgba(0,0,0,0.06)] border-x border-t border-brand/25 p-2 sm:p-3 relative overflow-hidden">
                                <div className="relative w-full rounded-xl overflow-hidden bg-brand/5 border border-brand/15">
                                    <Image
                                        src={BOTTOM_CARD.image}
                                        alt={BOTTOM_CARD.imageAlt}
                                        width={1200}
                                        height={800}
                                        className="w-full h-auto object-cover object-top opacity-90 transition-opacity duration-300 group-hover:opacity-100"
                                    />
                                </div>
                            </div>
                        </div>
